...
public class LightningAnimator : MonoBehaviour
{
    ...
    float FrameTimer = 0f; // Frame timer reference
    float OscillateTimer = 0f; // Beam oscillation timer reference
    ...
    void Raycast()
    {
        ...
        if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask))
        {
            ...
            if (rayImpact)
                rayImpact.position = hitPoint.point - transform.forward * 1f;
        }
    ...
    void Update()
    {
        ...
        if (Oscillate)
        {
            ...
            if (OscillateTimer > OscillateTime)
            {
