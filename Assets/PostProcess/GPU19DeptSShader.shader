...
Shader "Hidden/Custom/GPU19DepthSShader"
{
    HLSLINCLUDE
        ...
        float4 Frag(VaryingsDefault i) : SV_Target
        {
            ...
            float depth = dot(float2(1.0, 1/255.0),dn_enc.zw)+5;
			
            float3 n = DecodeViewNormalStereo(dn_enc);
			float3 display_n = 0.5 * (1 + n);
            origina.r = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoord+depth*float2(n.x*shh / _ScreenParams.x,n.y*shh / _ScreenParams.y)).r;
            origina.g = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoord-depth*float2(n.x*shh / _ScreenParams.x,n.y*shh / _ScreenParams.y)).g;
            origina.b = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoord+depth*float2(n.x*-shh / _ScreenParams.y,n.y*shh / _ScreenParams.x)).b;
